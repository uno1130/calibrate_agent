...
class xArmTransform:
    ...
    def __init__(self, initpos: list, initrot: list, initangle: list):
        ...
        self.__initX, self.__initY, self.__initZ = float(initpos[0]), float(initpos[1]), float(initpos[2])
        self.__initRoll, self.__initPitch, self.__initYaw = float(initrot[0]), float(initrot[1]), float(initrot[2])
